...
class BLELNClient {
public:
    // start: włącza BT i thread RX; name to local device name (używamy w advertising, ale klient nie musi)
    void start(const std::string& name, std::function<void(const std::string&)> onServerResponse);
    void stop();

    // Skanuj przez ~durationMs; filtr po UUID usługi; onResult(NULL) oznacza timeout/brak
    ...
    // Nawiąż połączenie do znalezionego urządzenia
    void beginConnect(const bt_addr_le_t* addr, const std::function<void(bool, int)>& onConnectResult);

    // Odkryj usługę/char’y i zasubskrybuj notyfikacje
    bool discover();

    // Wykonaj handshake (blokujący; jak w Twojej wersji)
    bool handshake();

    // Wyślij zaszyfrowaną wiadomość (DATA_RX write)
...
// --- Auth (passkey 123456 w razie pytania)
